...
- ⛔ **Sidebar classique : recherche restreinte à `.mainDrawer` UNIQUEMENT (corrigé en v3.8.1.0).** Élargir la recherche d'anchor au document avec `closest('… [class*="navMenuOption"] …')` faisait matcher la **sidebar du dashboard admin** (qui utilise aussi `a.navMenuOption`), plaçant l'entrée « Mes messages » sous « Tableau de bord » côté admin ; et pour les users (pas de dashboard) l'absence d'anchor matchant faisait `return false` → plus aucune entrée. Le bon scope est `document.querySelector('.mainDrawer')` (avec fallback `.mainDrawer-scrollContainer` pour certains skins). Fallback en l'absence d'anchor user : insertion APRÈS le dernier `.navMenuOption` du drawer (jamais un `appendChild` brut au scrollContainer, qui plaçait le bouton au milieu des bibliothèques dans certains skins — premier bug 3.7.5/6).
