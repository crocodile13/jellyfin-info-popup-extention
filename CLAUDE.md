...
├── dist/
├── .github/workflows/
│   ├── build.yml                ← CI : tests xUnit + builds matrix jf10.10/jf10.11 (v4.2.0.0)
│   └── release.yml              ← Manual workflow_dispatch fallback
├── tests/                            ← v4.1.1.0
...
└── Jellyfin.Plugin.InfoPopup/
    ...
    └── Web/
        ├── client.js                     ← loader parallèle (~85 lignes)
        ├── ip-i18n.js                    ← détection langue + dictionnaires (8 langues)
        ├── ip-utils.js                   ← utilitaires partagés
        ├── ip-styles.js                  ← CSS idempotent dans <head>
        ├── ip-admin.js                   ← entry config admin (init + toast/confirm + canPublish)
        ├── ip-admin-editor.js            ← markdown ↔ HTML + WYSIWYG + toolbar (v4.2.0.0)
        ├── ip-admin-targets.js           ← cache users + picker ciblage (v4.2.0.0)
        ├── ip-admin-permissions.js       ← onglet Droits + enhanceSelect (v4.2.0.0)
        ├── ip-admin-settings.js          ← onglets Paramètres + Réponses + Maintenance (v4.2.0.0)
        ├── ip-admin-messages.js          ← onglet Messages + CRUD + edit mode + modal Lectures (v4.2.0.0)
...
### Modular JS architecture (v0.6+, splitté v4.2.0.0)
`client.js` is a lightweight loader (~85 lines) that injects 11 modules via dynamic `<script async=false>` tags in a DocumentFragment (parallel fetch, ordered execution — v3.8.4.0). Ordre : `ip-i18n.js` → `ip-utils.js` → `ip-styles.js` → 5 sous-modules `ip-admin-*.js` → `ip-admin.js` (entry) → `ip-popup.js` → `ip-user.js`. All inter-module communication goes through the `window.__IP` namespace (IIFE pattern: `(function(ns){ ... }(window.__IP = window.__IP || {}))`). **Helpers admin internes** vivent dans `window.__IP.__admin` (cross-module via `ns.__admin.fnName`) ; l'API publique réutilisée par `ip-user.js` (`markdownToHtml`, `htmlToMarkdown`, `applyWysiwygFormat`) reste sur `ns.` directement pour rétrocompat.

**Split ip-admin.js → 6 fichiers (v4.2.0.0)** : avant, `ip-admin.js` faisait 2457 lignes en une seule IIFE et concentrait toute la logique de la page de configuration admin (messages tab, settings tab, replies tab, droits tab, éditeur WYSIWYG, target picker, toolbar formatage, modal Lectures, etc.). Désormais splitté par concern :
- `ip-admin-editor.js` (~375) : `markdownToHtml`, `htmlToMarkdown`, `syncWysiwygToTextarea`, `setEditorMode`, `applyFormat`, `toggleListLines`, etc.
- `ip-admin-targets.js` (~225) : `fetchUsers` (cache 5min), `renderTargetPicker`, `getSelectedTargetIds`, `setTargetPickerIds`, `resolveUserNames`
- `ip-admin-permissions.js` (~603) : `enhanceSelect`, `buildPermCard`, `loadPermissions`, ROLES const
- `ip-admin-settings.js` (~389) : `reloadSettingsValues`, `initSettingsTab`, `initMaintenanceSection`, `loadReplies` (utilise `admin.enhanceSelect`)
- `ip-admin-messages.js` (~502) : `renderMessages`, `loadMessages`, `publishMessage`, `deleteSelected`, `enterEditMode`, `exitEditMode`, `openViewsModal`, `updateSelectionUI`
- `ip-admin.js` (~477) : entry + orchestrateur `initConfigPage`, `checkConfigPage`, `applyStaticTranslations`, `initTabs`, `initSearchFilter`, `showToast`, `showConfirm`, `canPublish`/`setRateLimitMs`

Total : 2571 lignes au lieu de 2457 (~115 lignes d'overhead pour les IIFE wrappers + JSDoc headers, négligeable). Aucune logique perdue — vérifié par cross-référence `admin.X` défini ↔ utilisé.
...
Native `<select>` open lists can't be themed (`<option>` ignores most CSS). `enhanceSelect(sel)` in `ip-admin-permissions.js` (depuis v4.2.0.0, expose via `ns.__admin.enhanceSelect` pour réutilisation par `ip-admin-settings.js` dans le filtre Réponses) hides the native `<select>` (`.ip-sel-native`, kept for value/state) and renders a themed button + listbox (`.ip-sel*` classes in `ip-styles.js`). It dispatches a native bubbling `change` so all existing `sel.value` reads and `change` listeners keep working. Exposes `sel.__ipSync()` (refresh label after a programmatic `.value=`) and `sel.__ipRefresh()` (rebuild options). The outside-click listener is added on open and removed on close (no SPA leak).
...
| GET | `/InfoPopup/{module}.js` | anonymous | JS modules — whitelist: `client.js`, `ip-i18n.js`, `ip-utils.js`, `ip-styles.js`, `ip-admin-editor.js`, `ip-admin-targets.js`, `ip-admin-permissions.js`, `ip-admin-settings.js`, `ip-admin-messages.js`, `ip-admin.js`, `ip-popup.js`, `ip-user.js` |
...
- ⛔ **Éditeur user « Mes messages → Envoyer » : mêmes helpers que l'admin (v3.8.8.0).** Le mode WYSIWYG/Raw côté user partage `ns.markdownToHtml` / `ns.htmlToMarkdown` / `ns.applyWysiwygFormat` exposés désormais par **`ip-admin-editor.js`** (depuis v4.2.0.0 — étaient dans `ip-admin.js` avant le split). **Ne JAMAIS** dupliquer ces fonctions dans `ip-user.js` — la conversion markdown ↔ HTML doit rester un point unique de vérité (sinon divergence du dialecte markdown supporté entre admin et user). `applyFormat` et `toggleListLines` restent volontairement dupliqués (ils sont déjà dans `ip-user.js` historiquement, indépendants du WYSIWYG). Si `ip-admin-editor.js` n'a pas chargé (cas dégénéré), des fallbacks `escHtml`/`textContent` empêchent un crash mais rendent sans formatage — le toolbar fonctionne quand même en mode Raw.
