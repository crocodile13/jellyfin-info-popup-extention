...
- ⛔ **Sidebar classique : NE JAMAIS limiter la recherche d'anchor à `.mainDrawer-scrollContainer` (corrigé en v3.8.0.0).** Sur les installations avec KefinTweaks / JellyfinEnhanced / autres skins, le `scrollContainer` peut ne contenir QUE les bibliothèques — l'anchor `a[href*="logout"]` n'y est pas, le fallback `container.appendChild` plaçait alors le bouton « Messages » au milieu de la section Média. Recherche élargie au document avec `closest('.mainDrawer, .mainDrawer-scrollContainer, [class*="navMenuOption"]')` pour vérifier que l'anchor est bien dans un drawer/menu. Pas de fallback `appendChild` qui pourrait atterrir n'importe où.
