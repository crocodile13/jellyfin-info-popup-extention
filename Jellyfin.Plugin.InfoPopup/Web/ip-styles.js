...
(function (ns) {
    ...
    function injectStyles() {
        ...
        s.textContent = [
            ...
            // Liste déroulante personnalisée (enhanceSelect) — remplace le <select> natif pour
            // contrôler entièrement l'apparence de la liste ouverte (les <option> natifs ne sont
            // pas stylables). Le <select> natif est conservé caché (.ip-sel-native) pour la valeur.
            '.ip-sel{position:relative;display:inline-block;vertical-align:middle}',
            '.ip-sel-native{position:absolute!important;width:1px;height:1px;padding:0;margin:-1px;overflow:hidden;clip:rect(0 0 0 0);border:0;opacity:0;pointer-events:none}',
            '.ip-sel-btn{display:inline-flex;align-items:center;gap:8px;min-width:130px;background-color:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.2);border-radius:4px;color:inherit;padding:6px 32px 6px 11px;font-size:.88rem;line-height:1.4;font-family:inherit;cursor:pointer;text-align:left;position:relative;transition:border-color .15s,background-color .15s}',
            '.ip-sel-btn::after{content:"";position:absolute;right:12px;top:50%;width:7px;height:7px;border-right:2px solid #bbb;border-bottom:2px solid #bbb;transform:translateY(-70%) rotate(45deg);transition:transform .15s}',
            '.ip-sel.ip-open .ip-sel-btn::after{transform:translateY(-20%) rotate(-135deg)}',
            '.ip-sel-btn:hover{background-color:rgba(255,255,255,.13);border-color:rgba(255,255,255,.4)}',
            '.ip-sel-btn:focus,.ip-sel.ip-open .ip-sel-btn{outline:none;border-color:var(--theme-primary-color,#00a4dc)}',
            '.ip-sel-label{flex:1;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}',
            '.ip-sel-list{position:absolute;z-index:10000;top:calc(100% + 4px);left:0;min-width:100%;max-height:280px;overflow-y:auto;background:#23272b;border:1px solid rgba(255,255,255,.18);border-radius:6px;box-shadow:0 8px 24px rgba(0,0,0,.5);padding:4px;display:none}',
            '.ip-sel.ip-open .ip-sel-list{display:block}',
            '.ip-sel-opt{padding:7px 12px;border-radius:4px;cursor:pointer;font-size:.88rem;white-space:nowrap;color:#e5e5e5;transition:background .1s}',
            '.ip-sel-opt:hover,.ip-sel-opt-focus{background:rgba(255,255,255,.1)}',
            '.ip-sel-opt-active,.ip-sel-opt-active.ip-sel-opt-focus,.ip-sel-opt-active:hover{background:var(--theme-primary-color,#00a4dc);color:#fff}',
            '.ip-perm-bulk button{font-size:.85rem;padding:5px 14px}',
            '.ip-perm-bulk-status{font-size:.82rem;opacity:.7}',
            // Barre d'outils Droits (sélection + édition groupée) + enregistrement global
            '.ip-perm-toolbar{margin-bottom:20px;padding:12px 16px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.1);border-radius:6px}',
            '.ip-perm-toolbar-row{display:flex;align-items:center;gap:10px;flex-wrap:wrap}',
            '.ip-perm-toolbar-row + .ip-perm-toolbar-row{margin-top:10px;padding-top:10px;border-top:1px solid rgba(255,255,255,.06)}',
            '.ip-perm-tool-btn{background:none;border:1px solid rgba(255,255,255,.2);border-radius:4px;color:inherit;font-size:.8rem;cursor:pointer;padding:4px 12px;opacity:.85;transition:opacity .15s,background .15s,border-color .15s}',
            '.ip-perm-tool-btn:hover{opacity:1;background:rgba(255,255,255,.08);border-color:rgba(255,255,255,.4)}',
            '.ip-perm-sel-count{font-size:.82rem;opacity:.6;margin-left:auto}',
            '.ip-perm-bulk-num{width:56px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.2);border-radius:4px;color:inherit;padding:4px 6px;text-align:center;font-size:.82rem}',
            '.ip-perm-card-sel{width:16px;height:16px;cursor:pointer;accent-color:var(--theme-accent-color,#00a4dc);flex-shrink:0}',
            ...
            '.ip-perm-card-role{flex-shrink:0}',
            '.ip-perm-card-limits{display:flex;align-items:center;gap:6px;font-size:.82rem;opacity:.7;flex-shrink:0}',
            '.ip-perm-card-limits input[type="number"]{width:50px;background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.2);border-radius:4px;color:inherit;padding:2px 4px;text-align:center;font-size:.82rem}',
            '.ip-perm-card-actions{display:flex;align-items:center;gap:8px;flex-shrink:0}',
            '.ip-perm-save-btn{font-size:.78rem;padding:4px 12px}',
            '.ip-perm-save-ok{font-size:.78rem;color:#4caf50;margin-left:4px}',
            '.ip-perm-save-err{font-size:.78rem;color:#cf6679;margin-left:4px}',
            '.ip-perm-details{display:none;padding:8px 16px 12px;border-top:1px solid rgba(255,255,255,.06);background:rgba(0,0,0,.1)}',
            '.ip-perm-details.open{display:flex;flex-wrap:wrap;gap:12px 20px}',
            '.ip-perm-details label{display:flex;align-items:center;gap:6px;font-size:.85rem;cursor:pointer;user-select:none;white-space:nowrap}',
            '.ip-perm-details input[type="checkbox"]{width:14px;height:14px;cursor:pointer;accent-color:var(--theme-accent-color,#00a4dc)}',
            ...
            '.ip-user-overlay-content{padding:24px;max-width:800px;margin:0 auto}',
